...
class RingSizeScanner extends Scanner {
    ...
    private static final Selector FIRST_REACHABLE = ChordSuccessorSelector.getInstance();
    private static final Selector LAST_REACHABLE = ChordPredecessorSelector.getInstance();
    ...
    public static int cycleLengthFrom(final PeerReference application_reference, final boolean forwards) throws InterruptedException {
        ...
        final Set<PeerReference> nodes_encountered = new HashSet<>(); // Record the nodes that have already been encountered.
        int cycle_length = 0;
        ...
        while (!Thread.currentThread()
            ...
            try {
                node = forwards ? proxy.pull(FIRST_REACHABLE)
                        .get()
                        .get(0) : proxy.pull(LAST_REACHABLE)
                        .get()
                        .get(0);
            }
            ...
            if (node.getKey()
                    .equals(application_reference.getKey())) {
                return cycle_length;
            } // If the node is the start node, then a cycle has been found.
            if (nodes_encountered.contains(node)) {
                return cycle_length;
    ...
    private PeerReference getFirstRunningPeer(final ApplicationNetwork network) {
        ...
        for (final ApplicationDescriptor descriptor : network) {
            if (descriptor.isInAnyOfStates(ApplicationState.RUNNING)) {
                return descriptor.getApplicationReference();
            }
        }
        return null;
    }

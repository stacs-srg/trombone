...
public class Clustering implements Comparable<Clustering> {
    ...
    private double silhouetteWidth;
     ...
     * @param matrix
     */
    public Clustering(final RealMatrix matrix) {

        this.matrix = matrix;
        numberOfElements = matrix.getRowDimension();
     ...
     * @param threshold
     */
    void refine(final double threshold) {
        ...
        for (int memberIndex = 0; memberIndex < currentCluster.size(); ++memberIndex) {
            ...
            if (similarityToItsCluster < threshold) {
                ...
                for (int otherClusterIndex = 0; otherClusterIndex < size(); ++otherClusterIndex) {
                    ...
                    double similarity = ((otherCluster.size() - 1 + 0d) / otherCluster.size()) * otherCluster.calculateElementToClusterAverageSimilarity(member);
    ...
    void calculateDB() {
        ...
        for (int i = 0; i < size() - 1; ++i) {
            ...
            for (int j = i + 1; j < size(); ++j) {
                ...
                double d_ij = 1 - matrix.getEntry(v_i, v_j);
    ...
    void calculateDunnIndex() {
        ...
        for (int i = 0; i < size() - 1; ++i) {
            ...
            for (int j = i + 1; j < size(); ++j) {
                ...
                double interClusterDistance = 1 - matrix.getEntry(centroidI, centroidJ);
    ...
    void calculateSilhouette() {
        ...
        silhouetteWidth = (sumOfSilhouettes) / matrix.getRowDimension();
     ...
     */
    double calculateSilhouette(final Cluster cluster, final int clusterIndex) {
        ...
        for (int i = 0; i < cluster.size(); ++i) {
            ...
            for (int k = 0; k < cluster.size(); ++k) {
                ...
                ai += 1 - matrix.getEntry(member, anotherMember);
            ...
            for (int otherClusterIndex = 0; otherClusterIndex < size(); ++otherClusterIndex) {
                ...
                for (int k = 0; k < otherCluster.size(); ++k) {
                    ...
                    bi += 1 - matrix.getEntry(member, otherClusterMember);
